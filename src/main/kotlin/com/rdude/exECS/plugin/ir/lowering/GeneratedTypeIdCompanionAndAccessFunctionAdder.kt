...
class GeneratedTypeIdCompanionAndAccessFunctionAdder(private val existingCompanions: MutableMap<IrClass, IrClass>) {
    ...
    private fun addIdPropertyTo(toCompanion: IrClass, inClass: IrClass, type: HasId): IrProperty {
        return toCompanion.createAndAddPropertyWithBackingField(
            name = "execs_generated_${type.idPropertyNamePrefix.toLowerCase()}_type_id_property_for_${inClass.kotlinFqName.asString().replace(".", "_")}",
    ...
    private fun addGetIdFunction(irClass: IrClass, companion: IrClass, idProperty: IrProperty, type: HasId) {
        val function = IR_FACTORY.buildFun {
            this.name = Name.identifier("get${type.idPropertyNamePrefix.capitalize()}TypeId")
